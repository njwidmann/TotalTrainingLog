   ...
   $(document).on("click",".ui-dialog-titlebar-close", function() {
    ...
    self.addPreviousExercise = function(previousExercise, day) {
        if (ko.toJS(previousExercise) === undefined){
            ...
            day.exercises.push(previousExercise);
            $(".jqxExpander").jqxExpander();
    	...
    	self.addWeightsSet = function(exercise) {
												if (ko.toJS(exercise.sets).length > 0) {
                                                    var tempSet = exercise.sets()[ko.toJS(exercise.sets).length - 1];
                                                } else {
                                                    var tempSet =
       ...
       self.addWeightsExercise = function(weightTraining) {
                    var tempExercise = weightTraining.exercises()[ko.toJS(weightTraining.exercises).length - 1];

                    weightTraining.exercises.push(tempExercise);

                }
       
       self.addSet = function(exercise) {
                    var tempLabels = exercise.sets()[ko.toJS(exercise.sets).length - 1];

                    exercise.sets.push(tempLabels);
                }
